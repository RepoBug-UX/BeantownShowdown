...
contract DFund {
     ...
     */
    constructor(address _creator, uint256 _fundingGoal, uint256 _duration, string[] memory _milestoneDescriptions) {
        CREATOR = _creator;
        FUNDING_GOAL = _fundingGoal;
        ENDBLOCK = block.number + _duration;
        for (uint256 i; i < _milestoneDescriptions.length; ++i) {
            milestones.push(milestone(_milestoneDescriptions[i], 0, false, false, ""));
        }
    }
     ...
     */
    function submit_milestone(string calldata _milestone_submission, uint256 _index)
        external
        onlyCreator
        onlyWhenFunded
        notFrozen
     ...
     */
    function claim_milestone(uint256 _index) external onlyCreator onlyWhenFunded notFrozen reentrancyGuard {
        ...
        (bool success,) = CREATOR.call{value: (address(this).balance / (milestones.length - claimedMilestones))}("");
     ...
     */
    function vote_on_milestone(uint256 _index) external onlyFunder onlyWhenFunded notFrozen {
